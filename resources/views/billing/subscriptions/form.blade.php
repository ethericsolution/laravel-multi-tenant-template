<x-app-layout>
    ...
    <x-slot name="header">
        <div class="p-4 space-y-6">
            <x-common.breadcrumb :links=$breadcrumbLinks :title=$title :goBackAction="route('billing.subscriptions.index')" />
        </div>
    </x-slot>

    <form
        action="{{ $subscription->id ? route('billing.subscriptions.update', $subscription) : route('billing.subscriptions.store') }}"
        method="POST">
        @csrf
        ...
        <div class="mt-4">
            ...
            <a href="{{ route('billing.subscriptions.index') }}" class="btn btn-soft">{{ __('Cancel') }}</a>
        </div>
    </form>
