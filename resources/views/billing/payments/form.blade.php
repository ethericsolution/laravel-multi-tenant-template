<x-app-layout>
    ...
    <x-slot name="header">
        <div class="p-4 space-y-6">
            <x-common.breadcrumb :links=$breadcrumbLinks :title=$title :goBackAction="route('billing.payments.index')" />
        </div>
    </x-slot>

    <form action="{{ $payment->id ? route('billing.payments.update', $payment) : route('billing.payments.store') }}"
        method="POST">
        @csrf
        @if (isset($payment->id))
            @method('PUT')
        ...
        <div class="mt-4">
            ...
            <a href="{{ route('billing.payments.index') }}" class="btn btn-soft">{{ __('Cancel') }}</a>
        </div>
    </form>
