<x-tenant-app-layout>
    ...
    <x-slot name="header">
        <div class="p-4 space-y-6">
            <x-common.breadcrumb :links=$breadcrumbLinks :title=$title :goBackAction="route('app.billing.invoices.index')" />
        </div>
    </x-slot>

    <form
        action="{{ $invoice->id ? route('app.billing.invoices.update', $invoice) : route('app.billing.invoices.store') }}"
        method="POST">
        @csrf
        @if (isset($invoice->id))
            @method('PUT')
        ...
        <div class="mt-4">
            ...
            <a href="{{ route('app.billing.invoices.index') }}" class="btn btn-soft">{{ __('Cancel') }}</a>
        </div>
    </form>
