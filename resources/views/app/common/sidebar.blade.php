@php
    ...
    $settingNav = [
        [
            'name' => __('Users'),
            'route' => route('app.users.index'),
            'active' => request()->routeIs('app.users.*'),
            'icon' => '<span class="icon-[tabler--users] size-4.5"></span>',
        ],
    ];

    $navigation = request()->is('manage/*') ? $settingNav : $mainNav;
@endphp
        ...
        <div class="flex h-full max-h-full flex-col">
            ...
            <div class="mt-auto space-y-3 p-4">
                @if (request()->is('manage/*'))
                    <a href="{{ route('app.dashboard') }}" class="btn btn-soft btn-primary w-full">
                        <span class="icon-[tabler--arrow-left] size-5"></span>
                        Go Back
                    </a>
                @else
                    <a href="{{ route('app.users.index') }}" class="btn btn-soft btn-primary w-full">
